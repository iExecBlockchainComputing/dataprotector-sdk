...
describe('dataProtectorCore.processProtectedData()', () => {
  ...
  it(
    ...
    async () => {
      if (ENV === 'bellecour-fork') {
        ...
        // publish free app orders
        await createAndPublishAppOrders(iexec, appAddress);
      }
  ...
  if (ENV === 'bellecour-fork') {
    describe('use voucher', () => {
      ...
      it(
        'should throw error if app is not sponsored by voucher',
        async () => {
      ...
      it(
        'should throw error if workerpool is not sponsored by voucher',
        async () => {
          const voucherType = await createVoucherType({
            description: 'test voucher',
            duration: 60 * 60,
      ...
      it(
        'should successfully create task when app and workerpool are sponsored & sufficient voucher amount',
        async () => {
          const voucherType = await createVoucherType({
            description: 'test voucher',
            duration: 60 * 60,
      ...
      it(
        'should successfully create a task when user deposits to cover the missing amount',
        async () => {
          const voucherType = await createVoucherType({
            description: 'test voucher',
            duration: 60 * 60,
