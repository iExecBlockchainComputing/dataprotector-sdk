...
- The following method errors are captured by an `APICallError` during service unavailability and thrown as a `WorkflowError`:
  `getGrantedAccess` - MarketCallError
  `grantAccess` - MarketCallError
  `processProtectedData` - MarketCallError, IpfsGatewayCallError
