...
async function main() {
    ...
    const newAddOnlyAppWhitelistTx =
        await addOnlyAppWhitelistRegistryContract.createAddOnlyAppWhitelist(owner.address);
    const transactionReceipt = await newAddOnlyAppWhitelistTx.wait();
    let addOnlyAppWhitelistContractAddress = transactionReceipt.logs.find(
    ...
    for (let k = 0; k < 2; k++) {
        ...
        for (let i = 0; i < 2; i++) {
            ...
            await tx2.wait();
            console.log('ProtectedData added to collection', protectedDataAddress);
            const setProtectedDataForSaleTx =
                await dataProtectorSharingContract.setProtectedDataForSale(
                    protectedDataAddress,
                    salePrice,
                );
