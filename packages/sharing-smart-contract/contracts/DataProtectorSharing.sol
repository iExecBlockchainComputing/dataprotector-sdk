    ...
    IProtectedDataSharing
{
    ...
    constructor(IExecPocoDelegate _proxy, IRegistry protectedDataRegistry_, IAppWhitelistRegistry appWhitelistRegistry_)
        ManageOrders(_proxy)
    {
    ...
    function consumeProtectedData(
        ...
        bool isInSubscription = collectionTokenId != 0 && _protectedDataDetails.inSubscription
        ...
        IexecLibOrders_v5.RequestOrder memory requestOrder = createRequestOrder(
            _protectedData, _app, _workerpoolOrder.workerpool, _workerpoolOrder.category, _contentPath
        );
        // if voucher ? voucher.matchOrder : pococDelegate.matchorder
        dealid =
            _pocoDelegate.matchOrders(_appOrder, _protectedDataDetails.datasetOrder, _workerpoolOrder, requestOrder);
    ...
    function _swapCollection(
        ...
        emit ProtectedDataTransfer(_protectedData, _collectionTokenIdTo, _collectionTokenIdFrom, address(_appWhitelist));
    }

    // Overide burn function from ERC721BurnableUpgradeable.
    // Enable to burn a collectionTokenID.
    function _update(address to, uint256 _collectionTokenId, address auth)
        internal
        virtual
        override
        returns (address)
     ...
     * =========================================================================
     */
    function updateEnv(string memory iexec_result_storage_provider_, string memory iexec_result_storage_proxy_)
        public
    ...
    function removeProtectedDataFromCollection(address _protectedData) public {
        ...
        collectionDetails[_collectionTokenId].size -= 1;
        _protectedDataRegistry.safeTransferFrom(address(this), msg.sender, uint256(uint160(_protectedData)));
    ...
    function subscribeTo(uint256 _collectionTokenId, uint48 _duration) public payable returns (uint48 endDate) {
        ...
        if (
            ...
            revert InvalidSubscriptionDuration(_collectionTokenId, _collectionDetails.subscriptionParams.duration);
        }
        ...
        endDate = uint48(block.timestamp) + collectionDetails[_collectionTokenId].subscriptionParams.duration;
        collectionDetails[_collectionTokenId].subscribers[msg.sender] = endDate;
        collectionDetails[_collectionTokenId].lastSubscriptionExpiration =
            uint48(Math.max(endDate, collectionDetails[_collectionTokenId].lastSubscriptionExpiration));
    ...
    function rentProtectedData(address _protectedData) public payable {
        ...
        uint48 endDate = uint48(block.timestamp) + protectedDataDetails[_protectedData].rentingParams.duration;
        ...
        protectedDataDetails[_protectedData].lastRentalExpiration =
            uint48(Math.max(endDate, protectedDataDetails[_protectedData].lastRentalExpiration));
        earning[ownerOf(_collectionTokenId)] += msg.value;
        emit NewRental(_collectionTokenId, _protectedData, msg.sender, endDate);
    }

    /// @inheritdoc IRental
    function setProtectedDataToRenting(address _protectedData, uint112 _price, uint48 _duration) public {
        uint256 _collectionTokenId = protectedDataDetails[_protectedData].collection;
        _checkCollectionOperator(_collectionTokenId);
        _checkProtectedDataNotForSale(_protectedData);
    ...
    function buyProtectedDataForCollection(
        ...
        delete protectedDataDetails[_protectedData];
        _swapCollection(_collectionTokenIdFrom, _collectionTokenIdTo, _protectedData, _appWhitelist);
        protectedDataDetails[_protectedData].appWhitelist = _appWhitelist;
    ...
    function buyProtectedData(address _protectedData, address _to) public payable {
        ...
        delete protectedDataDetails[_protectedData];
        _protectedDataRegistry.safeTransferFrom(address(this), _to, uint256(uint160(_protectedData)));
        earning[ownerOf(_collectionTokenIdFrom)] += msg.value;
