...
export async function deploySCFixture() {
    ...
    const addOnlyAppWhitelistRegistryContract = await upgrades.deployProxy(
        AddOnlyAppWhitelistRegistryFactory,
        {
            kind: 'transparent',
        },
    );
...
export async function addProtectedDataToCollection() {
    ...
    const addOnlyAppWhitelistTokenId = transactionReceipt.logs.find(
        ({ eventName }) => eventName === 'Transfer',
    )?.args.tokenId;
    const addOnlyAppWhitelistContractAddress = ethers.getAddress(
        ethers.toBeHex(addOnlyAppWhitelistTokenId),
    );
