...
describe('Sale', () => {
  ...
  describe('setProtectedDataForSale()', () => {
    it('should set the protectedData for sale', async () => {
      ...
      const saleParams = (await dataProtectorSharingContract.protectedDataDetails(protectedDataAddress))[5];
    ...
    it('should emit ProtectedDataAddedForSale event', async () => {
      ...
      await expect(
        dataProtectorSharingContract.connect(addr1).setProtectedDataForSale(protectedDataAddress, priceParam),
      )
    ...
    it('should revert if the protectedData is currently available in subscription', async () => {
      ...
      await expect(
        dataProtectorSharingContract.connect(addr1).setProtectedDataForSale(protectedDataAddress, priceParam),
      ).to.be.revertedWithCustomError(dataProtectorSharingContract, 'ProtectedDataAvailableInSubscription');
    });
    ...
    it('should revert if the protectedData is available for renting', async () => {
      ...
      await expect(
        dataProtectorSharingContract.connect(addr1).setProtectedDataForSale(protectedDataAddress, priceParam),
      ).to.be.revertedWithCustomError(dataProtectorSharingContract, 'ProtectedDataAvailableForRenting');
    });
  ...
  describe('removeProtectedDataForSale()', () => {
    it('should remove protectedData for sale', async () => {
      const { dataProtectorSharingContract, protectedDataAddress, addr1 } =
        await loadFixture(addProtectedDataToCollection);
      await dataProtectorSharingContract.connect(addr1).removeProtectedDataForSale(protectedDataAddress);
    ...
    it('should emit ProtectedDataRemovedFromSale event', async () => {
      ...
      await expect(dataProtectorSharingContract.connect(addr1).removeProtectedDataForSale(protectedDataAddress))
    ...
    it('should revert if the user does not own the collection', async () => {
      ...
      await expect(
        dataProtectorSharingContract.connect(notCollectionOwner).removeProtectedDataForSale(protectedDataAddress),
      ).to.be.revertedWithCustomError(dataProtectorSharingContract, 'NotCollectionOperator');
    });
    ...
    it('should revert if the protectedData is not in the collection', async () => {
      ...
      await expect(
        dataProtectorSharingContract.connect(addr1).removeProtectedDataForSale(protectedDataAddress),
      ).to.be.revertedWithCustomError(dataProtectorSharingContract, 'ERC721NonexistentToken');
    });
  });
  ...
  describe('buyProtectedData()', () => {
    it('should transfer the protectedData to the buyer', async () => {
      ...
      await dataProtectorSharingContract.connect(addr1).setProtectedDataForSale(protectedDataAddress, priceParam);
      await pocoContract.connect(addr2).approve(await dataProtectorSharingContract.getAddress(), priceParam);
      await pocoContract.connect(addr2).deposit({
    ...
    it('should emit ProtectedDataSold event', async () => {
      ...
      await expect(
        dataProtectorSharingContract.connect(addr2).buyProtectedData(protectedDataAddress, addr2.address, priceParam),
      )
    ...
    it('should make a buy with an approveAndCall', async () => {
      const {
      ...
      const callData = DataProtectorSharingFactory.interface.encodeFunctionData('buyProtectedData', [
        protectedDataAddress,
        addr2.address,
        priceParam,
      ]);
      ...
      await pocoContract
        .connect(addr2)
        .approveAndCall(await dataProtectorSharingContract.getAddress(), priceParam, callData);
