...
describe('AddOnlyAppWhitelist', () => {
    describe('addApp', () => {
        it('should add an app correctly', async () => {
            ...
            const addOnlyAppWhitelistContractAddress = ethers.toBeHex(
                specificEventForPreviousTx.args?.tokenId,
            );
            const addOnlyAppWhitelistContractFactory =
                await ethers.getContractFactory('AddOnlyAppWhitelist');
            const addOnlyAppWhitelistContract = addOnlyAppWhitelistContractFactory.attach(
    ...
    describe('owner', () => {
        it('should share the same owner address between addOnlyAppWhitelist & whitelistRegistry state', async () => {
            const { addOnlyAppWhitelistRegistryContract, addr1 } =
                await loadFixture(deploySCFixture);
            const newAddOnlyAppWhitelistTx =
                await addOnlyAppWhitelistRegistryContract.createAddOnlyAppWhitelist(addr1.address);
            const transactionReceipt = await newAddOnlyAppWhitelistTx.wait();
    ...
    describe('transferOwnership', () => {
        it('should transfer the addOnlyAppWhitelist and have a coherent state between addOnlyAppWhitelist & the whitelistRegistry', async () => {
            const { addOnlyAppWhitelistRegistryContract, addr1, addr2 } =
                await loadFixture(deploySCFixture);
            const newAddOnlyAppWhitelistTx =
                await addOnlyAppWhitelistRegistryContract.createAddOnlyAppWhitelist(addr1.address);
            const transactionReceipt = await newAddOnlyAppWhitelistTx.wait();
        ...
        it('should transfer the addOnlyAppWhitelist from an authorized operator and have a coherent state between addOnlyAppWhitelist & the whitelistRegistry', async () => {
            const { addOnlyAppWhitelistRegistryContract, addr1, addr2, addr3 } =
                await loadFixture(deploySCFixture);
            const newAddOnlyAppWhitelistTx =
                await addOnlyAppWhitelistRegistryContract.createAddOnlyAppWhitelist(addr1.address);
            const transactionReceipt = await newAddOnlyAppWhitelistTx.wait();
            ...
            await addOnlyAppWhitelistRegistryContract
                .connect(addr1)
                .approve(addr3.address, addOnlyAppWhitelistTokenId);
